...
describe('transfer', () => {
  beforeEach(() => {
    ...
    fillManager!.listenOnFulfillment = vi.fn().mockResolvedValue(undefined);
  });
  ...
  describe('execute()', () => {
    it('triggers all protocol relevant functions', async () => {
      ...
      expect(fillManager!.listenOnFulfillment).toHaveBeenCalledTimes(1);
    });
  });
  ...
  describe('waitForRequestEvent()', () => {
    ...
    it('connects to the source chain', async () => {
      ...
      const transfer = new TestTransfer(data);
      const getRequestIdentifier = vi.spyOn(requestManager, 'getRequestIdentifier');

      await transfer.waitForRequestEvent();

      // TODO improve by spy on constructor of JsonRpcProvider
      expect(getRequestIdentifier).toHaveBeenCalledTimes(1);
      expect(getRequestIdentifier.mock.calls[0][0].connection.url).toBe('https://source.rpc');
    });
  ...
  describe('waitForFulfillment()', () => {
    ...
    it('connects to the target chain', async () => {
      ...
      const transfer = new TestTransfer(data);
      const listenOnFulfillment = vi.spyOn(fillManager, 'listenOnFulfillment');

      await transfer.waitForFulfillment();

      // TODO improve by spy on constructor of JsonRpcProvider
      expect(listenOnFulfillment).toHaveBeenCalledTimes(1);
      expect(listenOnFulfillment.mock.calls[0][0].connection.url).toBe('https://target.rpc');
    });
    ...
    it('uses the correct parameters to wait for the request fill', async () => {
      ...
      expect(fillManager.listenOnFulfillment).toHaveBeenCalledTimes(1);
      expect(fillManager.listenOnFulfillment).toHaveBeenLastCalledWith(
        expect.any(JsonRpcProvider),
        1,
        '0xFillManager',
        1, // TODO: this currently depends on the `__mocks__/@ethersproject/providers` implementation
      );
    });
  });
